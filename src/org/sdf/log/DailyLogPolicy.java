...
public class DailyLogPolicy implements ILogPolicy {
	String name;
	String dir;
	String date = "";
	String mon = "";
	...
	public DailyLogPolicy(String dir, String name) {
		this.dir = dir;
		this.name = name;
	...
	public void backup() {
		if (out != null) {
			...
			if (file.exists()) {
				for (int i = 0;; i++) {
					File bfile = new File(dname, name + "." + date + "." + i
							+ ".log");
					if (!bfile.exists()) {
						file.renameTo(bfile);
						Log.cfg.info(fname + " --> " + name + "." + date + "."
								+ i + ".log " + " backup");
						break;
					}
				}
	...
	public String toString() {
		String dname = dir + "/" + mon;
		String fname = name + "." + date + ".log";
		return "[" + name + "] : DailyLogPolicy : " + dname + "," + fname;
	}
